...
The pipeline isn’t built for automation or trading. It’s designed to provide structure when analyzing how operational signals might connect to broader market behavior.
...
It reflects the way analysts work when the information isn’t perfect: you organize what you have, clean it, quantify it, and look for movement that stands out.
...
├── requirements.txt
│
├── src/ # Core logic for ingesting and processing AIS + market data
│ ├── ingest.py
│ ├── transforms.py
│ ├── signals.py
│ └── utils.py
│
├── scripts/ # Helper utilities for data checks and conversions
│
├── examples/ # Small sample datasets and demos (no large AIS files)
│
...
- Breaking down workflows into modular steps  
- Summarizing findings in plain, understandable language  

---

This project wasn’t built for show. It grew out of trying to understand how operational signals — especially vessel movements and port behavior — connect to shifts downstream. It’s simply a structured workflow that became polished enough to share.
